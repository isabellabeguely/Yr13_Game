...
heart = pygame.image.load(resource_path("assets/heart-icon.png"))
...
CONGRATULATIONS_FONT = pygame.font.Font(resource_path("assets/font.otf"), 130)  
...
class freeplay_rungame:
    ...
    def __init__(self, screen,font_letter,font_word,startx,starty,word):
        ...
        self.used_words = set()  # to track used words
        self.word = word 
            ...
            if visible and math.hypot(x-m_x,y-m_y) < RAD:
                ...
                if ltr in self.word:
                    ...
                    if all (ltr in self.guessed for ltr in self.word):
                        #used_words = [self.word]  #    store the current word as used
                        #next_word = None

                        remaining_words = [w for w in words if w not in self.used_words]
                        
                        
                        if remaining_words:
                            next_word = random.choice(remaining_words)
                            self.word = next_word
                            self.used_words.add(next_word)  # Add new word to used words
                            self.guessed = []  # Reset guessed letters
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                mouse_pos = pygame.mouse.get_pos()
                if showing_rules:
                    showing_rules = False  # check if rules are being shown and hide them
        ...
        # Draw the game elements
        if game_over:
            back_button_rect = draw_game_over_screen(screen,None,game_over_reason) #  ✅  Draw game over screen and get back button rect
        elif showing_rules:  
            show_rules(screen)
        else:
            grid.draw_letters()
