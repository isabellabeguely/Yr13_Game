            ...
            if visible and math.hypot(x-m_x,y-m_y) < RAD:
                ...
                if ltr in self.word:
                    ...
                    if all (ltr in self.guessed for ltr in self.word):
                        used_words = [self.word]  # store the used words
                        next_word = None # Initialize next_word to None

                        remaining_words = [w for w in words if w not in used_words]
                        if remaining_words:
                            next_word = random.choice(remaining_words) # Get a random word from the remaining words
