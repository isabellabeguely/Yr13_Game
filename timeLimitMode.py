...
def draw_game_over_screen(screen, font):
    ...
    # Set text content and shadow offset
    text = "GAME OVER"
    shadow_offset = 6
...
while running:
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.USEREVENT: 
            if not game_over:
                counter -= 1
                text = str(counter) if counter > 0 else 'GAME OVER' #the below text shows when the time runs out
            if counter <= 0:
                game_over = True
        
    if game_over:
        
        draw_game_over_screen(screen, GAMEOVER_FONT)
